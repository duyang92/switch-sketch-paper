...
uint64_t B[BUCKET_NUM][1]{0};//array of buckets
...
//        srand((unsigned)time(NULL));
        ...
        ofstream report("./switchsketch64-zby -Report.txt");
        double AAEOfHH = 0;
        double AREOfHH = 0;
        for (unsigned int idx = 0; idx < estimatedFlowSizesVector.size(); idx++) {
        ...
        ofstream out("./switchsketch64-zby -m64 -O.txt", ios::app);
        out << "****************************-----test " << i << endl;
        ...
        ofstream real("./switchsketch64-zby -Real.txt");
        AAEOfHH = 0;
        AREOfHH = 0;
        for (unsigned int idx = 0; idx < actualFlowSizesVector.size(); idx++) {
