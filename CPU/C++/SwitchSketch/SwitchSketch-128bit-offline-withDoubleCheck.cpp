...
#define BUCKET_NUM 6400 //the number of bucket 6400*128bits=100KB
...
struct HashFunc {
    uint32_t operator()(const uint8_t *key) const {
        uint32_t hashValue = BKDRHash(key, KEY_SIZE, 131);
        return hashValue;
    }
};
...
void insert(int packet_cnt) {
    ...
    for (i = 0; i < packet_cnt; i++) {
        if (i & 1) {
            hash = BKDRHash(keys[i], KEY_SIZE, 131);
        } else {
            hash = BKDRHash(keys[i], KEY_SIZE, 13131);
        }
...
uint32_t query(uint8_t *key, uint32_t index) {
    uint16_t tmp_fp, tmp_counter;
...
void getEstimatedFlowSizes(double epsilon, vector <pair<uint8_t *, unsigned int>> actualFlowSizesVector,
    ...
    for (int i = 0; i < actualFlowSizesVector.size(); i++) {
        ...
        uint32_t result1 = query(key, 0);
        uint32_t result2 = query(key, 1);

        if (result1 != 0 || result2 != 0) {
            double checkVal = 1.0 - ((double) min(result1, result2) / max(result1, result2));

            if (checkVal < epsilon && result1 + result2 >= THRESHOLD) {
                estimatedFlowSizes[key] = result1 + result2;
            }
        }
    }
}
...
ReadInTraces(const char *traceFilePath, unordered_map<uint8_t *, uint32_t, HashFunc, CmpFunc> &actualFlowSizes) {
    ...
    while (fread(temp, 1, KEY_SIZE, fin) == KEY_SIZE) {
        ...
        if (fread(temp, 1, KEY_SIZE, fin) != KEY_SIZE) {
            break;
        }
    }
    printf("Successfully read in %s, %u items\n", traceFilePath, count);
...
int main() {
    ...
    uint32_t packet_cnt = ReadInTraces("../data/49.dat", actualFlowSizes); /
...
//        srand((unsigned)time(NULL));
        ...
        ofstream report("./switchsketch128-doubleCheck -Report.txt");
        double AAEOfHH = 0;
        double AREOfHH = 0;
        ...
        for (unsigned int idx = 0; idx < estimatedFlowSizesVector.size(); idx++) {
            ...
            report << "Rep-" << idx << "\t" << result1 << "|" << result2 << "|" << estimatedSize << "\t" << actualSize
        ...
        ofstream out("./switchsketch128-doubleCheck -m64 -O.txt", ios::app);
        out << "****************************-----test " << i << endl;
        ...
        ofstream real("./switchsketch128-doubleCheck -Real.txt");
        AAEOfHH = 0;
        AREOfHH = 0;
        for (unsigned int idx = 0; idx < actualFlowSizesVector.size(); idx++) {
            ...
            if (actualSize >= THRESHOLD) {
                ...
                real << "Real-" << idx << "\t" << result1 << "|" << result2 << "|" << estimatedSize << "\t"
                     << actualSize << "\t" << setiosflags(ios::fixed | ios::right) << setprecision(6)
                     << AAEOfHH / (idx + 1) << "\t" << AREOfHH / (idx + 1) << endl;
            } else {
                break;
            }
        }
