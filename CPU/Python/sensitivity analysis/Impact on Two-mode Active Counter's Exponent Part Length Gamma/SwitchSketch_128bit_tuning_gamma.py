...
def Adjust(C3_old,C2_old,C1_old,C3_new,C2_new,C1_new):
    ...
    if C3_new == C3_old and C2_new == C2_old + 1:
        ...
        # if [len(Level[1][bucket_index1]), len(Level[2][bucket_index1]), len(Level[3][bucket_index1])] not in lst_legal:
    ...
    for gamma in [3,4,5]:
        for memorysize in [100]:
            for testnum in range(10):
                ...
                b = 1.08
                print("=============================")
                print("length of the two-mode active counter's exponent part:{}".format(gamma))
                lst_PR = []
                lst_RR = []
                T = 2**14
                top = int(4/3*T)
                ...
                for item in F:
                    ...
                    if query_result:
                        if query_result[1] >= 2:
                            if not isinstance(Level[query_result[1]][bucket_index1][query_result[0]][1],int):
                                set_HH.add(item)
