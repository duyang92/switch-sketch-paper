...
def Adjust(C3_old,C2_old,C1_old,C3_new,C2_new,C1_new):
    ...
    if C3_new == C3_old and C2_new == C2_old + 1:
        ...
        # if [len(Level[1][bucket_index1]), len(Level[2][bucket_index1]), len(Level[3][bucket_index1])] == [1,2,0]:
    ...
    # reconstruct a new l3
        ...
        #clear l2 cells only
        # 0 1 2 - 1 0 2
        elif clear_num_l1 == 0 and clear_num_l2 == 1:
            lst_index[bucket_index1][0] += 1
            lst_index[bucket_index1][1] -= 1
            # clear the original record
            for index, item in enumerate(Level[2][bucket_index1]):
        ...
        # 0 2 1 --> 1 1 0
        elif clear_num_l2 > 0 and clear_num_l1 > 0:
            for index, item in enumerate(Level[2][bucket_index1]):
                if item != 0:
                    if item[0] == FP_level2:
                        Level[2][bucket_index1].pop(index)
...
def Switch(k, C3, C2):
    ...
    if k == 1:
        ...
        if result != 99:
            ...
            if random.random() < gailv_minus/16:
...
if __name__ == '__main__':
    ...
    for topK in [512,1024]:
        ...
        for top in [1000]:
                ...
                if query_result:
                    if query_result[1] == 2:
                        if isinstance(Level[query_result[1]][bucket_index1][query_result[0]][1],int):
                            if Level[query_result[1]][bucket_index1][query_result[0]][1] == T//16:
                                set_HH.add(item)
            ...
            #test
            lst_tem2 = []
            lst_legal = [[5, 0, 0], [3, 1, 0], [1, 2, 0], [0, 3, 0], [2, 0, 1], [0, 1, 1], [0, 0, 2]]
            ...
            for key in set_HH:
                ...
                if query_result1 != 0:
                    ...
                    if query_level == 3:
                        ...
                        value_result += Level[2][ID1][query_index][1]*16
                    elif query_level == 1:
                        value_result += Level[1][ID1][query_index][1]*16
                    else:
                        continue
