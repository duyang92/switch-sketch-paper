...
def Adjust(C3_old,C2_old,C1_old,C3_new,C2_new,C1_new):
    ...
    if C3_new == C3_old and C2_new == C2_old + 1:
        ...
        if [len(Level[1][bucket_index1]), len(Level[2][bucket_index1]), len(Level[3][bucket_index1])] == [7,1,0]:
        ...
        #l2 exponential decay
            ...
            if random.random() < gailv_minus:
                if isinstance(Level[2][bucket_index1][min_pos][1], int):
                    ...
                    if Level[2][bucket_index1][min_pos][1] < 2 ** (counter_bit_level1):
                        Level[2][bucket_index1][min_pos][0] = FP_level2
                        Level[2][bucket_index1][min_pos][1] = 2 ** (counter_bit_level1)
                        for index, item in enumerate(Level[1][bucket_index1]):
                            if item != 0:
    ...
    for topK in [64, 128, 256]:
        ...
        for memorysize in [50]:
            ...
            for top in [1000]:
                ...
                #test
                lst_tem2 = []
                lst_legal = [[7, 0, 0], [6, 1, 0], [4, 2, 0], [3, 3, 0], [1, 4, 0], [0, 5, 0], [5, 0, 1], [4, 1, 1], [2, 2, 1], [1, 3, 1], [3, 0, 2], [2, 1, 2], [0, 2, 2], [1, 0, 3], [0, 1, 3]]
