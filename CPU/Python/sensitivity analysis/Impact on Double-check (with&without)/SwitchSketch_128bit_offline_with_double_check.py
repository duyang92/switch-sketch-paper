...
def Adjust(C3_old,C2_old,C1_old,C3_new,C2_new,C1_new):
    ...
    if C3_new == C3_old and C2_new == C2_old + 1:
        ...
        # if [len(Level[1][bucket_index1]), len(Level[2][bucket_index1]), len(Level[3][bucket_index1])] == [2, 1, 0]:
        ...
        for top in [1000]:
            ...
            for item in tqdm(F):
                if random.random() < 1 / 2:
                    bucket_index1 = mmh3.hash(str(item), SEED[0]) % BUCKET_NUM
                else:
                    bucket_index1 = mmh3.hash(str(item), SEED[2]) % BUCKET_NUM

                FP_level1 = mmh3.hash(str(item),SEED[1]) % (2 ** fp_bit_level1)
                FP_level2 = mmh3.hash(str(item),SEED[3]) % (2 ** fp_bit_level2)
                FP_level3 = mmh3.hash(str(item),SEED[4]) % (2 ** fp_bit_level3)
            ...
            for i in l.items():
                ...
                ID2 = mmh3.hash(i[0], SEED[2]) % BUCKET_NUM
                ...
                query_result2 = Query(i[0], ID2, FP_l1, FP_l2, FP_l3)
                ...
                if query_result1 != 0:
                    ...
                    if query_level == 3:
                        if not isinstance(Level[3][ID1][query_index][1],int):
                            value1 += Level[3][ID1][query_index][1][1]*(
                        ...
                        value1 += 2 * Level[1][ID1][query_index][1]
                    else:
                        continue

                if query_result2 != 0:
                    query_index = query_result2[0]
                    query_level = query_result2[1]
                    if query_level == 3:
                        if not isinstance(Level[3][ID2][query_index][1], int):
                            value2 += Level[3][ID2][query_index][1][1] * (
                                    2 ** (Level[3][ID2][query_index][1][0] + gamma))
                        else:
                            value2 += Level[3][ID2][query_index][1]
                    elif query_level == 2:
                        value2 += Level[2][ID2][query_index][1]
                    elif query_level == 1:
                        value2 += 2 * Level[1][ID2][query_index][1]
                    else:
                        continue
                ...
                if value1 != 0 and value1 >= value2:
                    C = 1 - value2 / value1
                elif value2 != 0 and value2 >= value1:
                    C = 1 - value1 / value2
                if C < theta:
                    value_result = value1 + value2

                if value_result > T:
                    result_overT.update({i[0]: value_result})
                result_all.update({i[0]: value_result})
