...
#define BUCKET_NUM 12800//the number of bucket 12800*128bits=200KB
...
ap_uint<128> B[BUCKET_NUM];
inline uint32_t BKDRHash(const uint8_t *str, uint32_t len, uint32_t seed) {
...
void setCellFP(ap_uint<128> &bucket, int level, uint16_t fp16, uint32_t cell_start_bit_idx) {
    uint32_t fp_mask;
    switch (level) {
        case 1: {
            fp_mask = FP_MASK1;
    ...
    bucket &= ~(((ap_uint<128>) fp_mask) << (cell_start_bit_idx));
    bucket |= ((ap_uint < 128 > )(fp16 & fp_mask)) << (cell_start_bit_idx);
}
void setCellCounter(ap_uint<128> bucket, int level, uint16_t newCounter, uint32_t cell_start_bit_idx) {
    uint32_t fp_len;
    uint32_t ct_mask;
    switch (level) {
        case 1: {
    ...
    bucket &= ~(((ap_uint<128>) ct_mask) << (cell_start_bit_idx));
    bucket |= ((ap_uint < 128 > )(newCounter & ct_mask)) << (cell_start_bit_idx);
}
void setCell(ap_uint<128> &bucket, int level, uint16_t fp16, uint16_t newCounter, uint32_t cell_start_bit_idx) {
    uint32_t fp_len;
    uint32_t fp_mask;
    uint32_t ct_mask;
    uint32_t cell_mask;
    ...
    bucket &= ~(((ap_uint<128>) cell_mask) << cell_start_bit_idx);
    bucket |= ((ap_uint < 128 > )((fp16 & fp_mask) | ((newCounter & ct_mask) << fp_len))) << (cell_start_bit_idx);
}
void clearCell(ap_uint<128> &bucket, int level, uint32_t cell_start_bit_idx) {
    uint32_t cell_mask;
    switch (level) {
        case 1: {
            cell_mask = CELL_MASK1;
...
void findMinCell(ap_uint<128> &bucket, int level, const int &num_lv1, const int &num_lv2, const int &num_lv3,
...
void
Switch(ap_uint<128> &bucket, int level, const int &num_lv_1, const int &num_lv_2, const int &num_lv_3, uint16_t fp16,
    ...
    for (j = META_LENGTH; j < start_lv2; j += CELL_LEN3) {
        temp_bucket |= (bucket) & (((ap_uint < 128 > )(CELL_MASK3)) << j);
    }
    i = start_lv2;
    //store fp16 when level==3
    if (level == 3) {
        temp_bucket |= (((ap_uint<128>) MIN_C_LV_3 << FP_LEN3) | (fp16 & FP_MASK3)) << i;
        i += CELL_LEN3;
    }
    //store level_2 flows
    for (usage_lv_2 = 0, j = start_lv2; j < start_lv1; j += CELL_LEN2) {
        ...
        if (tmp_counter && (tmp_fp != (fp16 & FP_MASK2))) {
            if (!min_counter) {
                ...
                temp_bucket |= (((ap_uint<128>) min_counter << FP_LEN2) | (min_f)) << i;
                i += CELL_LEN2;
                min_f = tmp_fp;
                min_counter = tmp_counter;
                usage_lv_2++;
            } else {
                temp_bucket |= (((ap_uint<128>) tmp_counter << FP_LEN2) | (tmp_fp)) << i;
                i += CELL_LEN2;
                usage_lv_2++;
            }
        }
    ...
    if (level == 2) {
        temp_bucket |= (((ap_uint<128>) MIN_C_LV_2 << FP_LEN2) | (fp16 & FP_MASK2)) << i;
        i = i + CELL_LEN2;
    }
    //store level_1 flows
    min_counter = 0;
    for (j = start_lv1; j < end_lv1; j += CELL_LEN1) {
        ...
        if (tmp_counter && (tmp_fp != (fp16 & FP_MASK1))) {
            if (!min_counter) {
                ...
                temp_bucket |= (((ap_uint<128>) min_counter << FP_LEN1) | (min_f)) << i;
                i += CELL_LEN1;
                min_f = tmp_fp;
                min_counter = tmp_counter;
                usage_lv_1++;
            } else {
                temp_bucket |= (((ap_uint<128>) tmp_counter << FP_LEN1) | (tmp_fp)) << i;
                i += CELL_LEN1;
                usage_lv_1++;
            }
        }
...
void insert(uint8_t *key) {
    packet_c++;
    uint32_t hash;
    if (packet_c & 1) {//Double Check
        hash = BKDRHash(key, KEY_SIZE, 131);
    } else {
        hash = BKDRHash(key, KEY_SIZE, 13131);
    }
    ...
    ap_uint<128> &bucket = B[bucket_index];
